...
Fs =20e6
Ts = 1 / Fs
lags = (np.arange(len(sig)) / Fs)*1000
...
def autocorr_circ(sig):
    X = fft(sig)
    X_conj = np.conj(X)
    y = ifft(X * X_conj)  # Autocorrelation of the input signal
    return np.abs(y)
...
#5 40Hz et 100Hz
#for 40Hz 
delay_40Hz  = rate_to_delay(40)*1000
delay_100Hz = rate_to_delay(100)*1000
...
def find_subvector_zoom(sig , min, max):
    ...
    index_min = int(delay_max / 1000 * Fs)
    index_max = int(delay_min / 1000 * Fs)

    print("indice min" , index_min)
    print("indice max" , index_max)

    subvector_auto       = autocorr_circ( sig )
    subvector_auto_zoom  = subvector_auto[index_min : index_max]
    #print(subvector_auto_zoom)
    subvector_lags_zoom  = lags[index_min : index_max]
    ...
    max_val = np.max(subvector_auto_zoom)
    max_index = np.argmax(subvector_auto_zoom)
    print("max_index" , max)
    max_lag = subvector_lags_zoom[max_index]
    print( " max_lag" , max_index)
    #print("index avec np.argmax" , np.argmax(subvector_auto_zoom))
    return max
